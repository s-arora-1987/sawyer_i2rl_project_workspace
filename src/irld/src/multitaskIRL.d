...
class NonParamMTIRL : MaxEntIrlZiebartApprox {
    ...
    out double [][] opt_weights, int dim, double [][] trueWeights, int[] true_assignments) { 
        ...
        foreach(traj_fe; fk_Xms_demonstration) {
            foreach (i; 0 .. mu_E_debugSingleTask.length) {
                mu_E_debugSingleTask[i] += traj_fe[i];
            }
        ...
        if (max_iter > 0) { 
            ...
            do {
                ...
                foreach (q; 0 .. this.max_d) {
                    ...
                    do {
                        ...
                        temp_weights_restarts = singleTaskUnconstrainedAdaptiveExponentiatedStochasticGradientDescent(
                            ...
                            error, 25, this.Ephi_thresh, q, false, 1);
                        restarts += 1;
                ...
                foreach (q; 0 .. this.max_d) {
                    lbavals.length = 0;
                    writeln("lba values for cluster ",q," w.r.t. policies for true reward weights ");
        ...
        writeln(n_corr_lrndass/learned_assignments.length,
            "endassignmentAccuracy","_",learned_assignments.length);

        avg_EVD = avg_EVD/(cast(double)fk_Xms_demonstration.length);
    ...
    double [] singleTaskUnconstrainedAdaptiveExponentiatedStochasticGradientDescent(double [] w,
        double nu, double err, size_t max_iter, double Ephi_thresh, int q, 
...
int main() {
	...
	if (algorithm == "DPMMEIRL") {
        ...
        double avd_EVD = irl.solve(mapToUse, model, initial, SAR, max_traj_length, lastWeights, 
            foundWeightsGlbl, reward.dim(), trueWeights, true_assignments); 
