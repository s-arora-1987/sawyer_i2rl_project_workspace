...
#    irl = mdp.solvers.IRLApprximateSolver(20, mdp.solvers.ValueIteration(50), 500)
    ...
    for i in range(observableStateLow, observableStateHigh + 1):
        for a in model.A():
            observableStates.append( (patrol.model.PatrolState(np.array([i, 0])), a) )
        for a in model.A():
            observableStates.append( (patrol.model.PatrolState(np.array([i, 1])), a) )
    
    rewardirl = patrol.reward.PatrolRewardIRL3(observableStates)
