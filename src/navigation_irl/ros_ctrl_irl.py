...
#					
#							for a2 in pmodel.A():
#						C		for (newS, newP) in pmodel.T(s2, a2).items():
#									if not (newS.location[0] == 0 and newS.location[1] <= 7 and newS.location[1] >= 6 ):
#										addState = True
#										for (l) in allStates:
...
#							foundChild = False
#					
#							for (newS, newP) in pmodel.T(s2, a2).items():
...
#	traj = samples
	...
	#print("traj_irl written. signalling abortrun in irlsolve() method")
	#abortRun()
	...
	def goNow(self):
		...
		if self.finalQ is not None and not self.goTime < sys.maxint:
			...
			try:
				...
				self.p.join()
				print("Got the queue items at: " + str(get_time()))
				...
				print("Start Search at",fromRospyTime(get_time() - calcStart), "BestTime", totalBestTime, "MaxValue", totalMaxValue)
				...
				print("Got GoTime of "  + str(self.goTime))
				
#				self.searchCount += 1
#				if (self.searchCount > 300):
#					abortRun();
			except Queue.Empty:
				#print("Queue.Empty WrapperIRL goNow")
				pass
	...
	def updateWithAlg(self, alg, patrollerdelay):
		...
		if not self.isSolving and self.policy is None:
			...
			try:
				...
				print("self.patPolicies = newStuff:"+str(self.patPolicies))
...
def handle_attacker(req):
	global lastActualPosition, BatchIRLflag
